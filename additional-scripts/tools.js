...
{
  let mac_regex = /^^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/gm;
  if(mac_regex.test(str))
    return true;
  else
    return false;
}
...
{
  host_regex = /^(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9])$/gm;
  if(host_regex.test(str))
    return true;
  else
    return false;
}
